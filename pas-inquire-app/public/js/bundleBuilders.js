...
export function buildFullBundle(v) {
  ...
  const bundle = {
    ...
    entry: [
      { fullUrl: `urn:uuid:${id.claim}`, resource: {
        ...
        identifier: [{ system: v.submitterTxnIdSystem, use: "usual", value: txn }],
      ...
      { fullUrl: `urn:uuid:${id.coverage}`, resource: coverageResource(v, id, true) },
      ...
      { fullUrl: `urn:uuid:${id.patient}`, resource: patientResource(v, id, true) },
      ...
      { fullUrl: `urn:uuid:${id.requestorOrg}`, resource: requestorOrg(v, id) },
      ...
      { fullUrl: `urn:uuid:${id.practitioner}`, resource: {
        ...
        identifier: [{ system: "http://hl7.org/fhir/sid/us-npi", value: v.practitionerNpi }],
...
export function buildMinimalBundle(v, opts = {}) {
  ...
  if (opts.withPaRef && v.priorAuthRefNumber) {
    itemExt.push(ext("extension-administrationReferenceNumber", { valueString: v.priorAuthRefNumber }));
  }
  ...
  const bundle = {
    ...
    timestamp: iso,
    entry: [
      { fullUrl: "urn:uuid:claim-1", resource: {
        ...
        ...(opts.withPaRef && v.priorAuthRefNumber ? { identifier: [{ system: v.submitterTxnIdSystem, value: v.priorAuthRefNumber }] } : {}),
      ...
      { fullUrl: "urn:uuid:patient-1", resource: patientResource(v, { patient: "patient-1" }, false) },
      { fullUrl: "urn:uuid:requestor-1", resource: requestorOrg(v, { requestorOrg: "requestor-1" }, true) },
      { fullUrl: "urn:uuid:insurer-1", resource: insurerOrg(v, { insurerOrg: "insurer-1" }, true) },
      { fullUrl: "urn:uuid:coverage-1", resource: coverageResource(v, { coverage: "coverage-1", patient: "patient-1", insurerOrg: "insurer-1" }, false) },
    ],
  };
...
function patientResource(v, id, full) {
  const profile = full
...
function coverageResource(v, id, full) {
  const ref = (key, val) => (full ? `${key}/${val}` : `urn:uuid:${val}`);
  return {
    ...
    identifier: [{ use: "official", value: v.memberIdValue, system: v.memberIdSystem, type: codeable("MB", undefined, "http://terminology.hl7.org/CodeSystem/v2-0203") }],
...
function requestorOrg(v, id, minimal) {
  const r = {
    ...
    identifier: [
      { system: "http://hl7.org/fhir/sid/us-npi", use: "official", value: v.providerNpi },
...
function memberId(v) { return { use: "official", value: v.memberIdValue, system: v.memberIdSystem, type: codeable("MB", "Member Match Identifier", "http://terminology.hl7.org/CodeSystem/v2-0203") }; }
